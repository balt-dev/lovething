...
function S:init()
	...
	self:addUI {
		...
		},
		{
			...
			{
				type = UI.NODE.COLUMNS,
				config = { background_color = { 0.5, 0, 0, 1 }, border_width = 3, border_color = { 0, 0, 0, 1 }, margin = 20 },
				{
					type = UI.NODE.DIV,
		...
		{}, -- Spacer nodes are empty
		{
			...
			UI.Scrollbox({
				{type = UI.NODE.DIV, config = {text = "Scrollbox"}},
				{type = UI.NODE.DIV, config = {text = "Scrollbox"}},
				{type = UI.NODE.DIV, config = {text = "Scrollbox"}},
				{type = UI.NODE.DIV, config = {text = "Scrollbox"}},
				{type = UI.NODE.DIV, config = {text = "Scrollbox"}},
			...
			UI.Button(function() error(":clueless:") end, {
				text = "Click to crash",
				background_color_idle = {1, 1, 1, 1},
				background_color_hover = {1, 0.8, 0.8, 1},
				background_color_click = {1, 0, 0, 1},
				text_color = {0, 0, 0, 1}
			})
		},
	}
